// DAL: 격차 리포트 read — 지역 현황 + STAGE_LINK + 조직 + 기회카드 묶음.
// 근거: ARCHITECTURE.md §5.2, FR-GR-01~05
// 목업 스코프는 지역 1곳("한빛구", OQ-01 이월)뿐이라 stageLinks/orgs는 전건을 그대로 묶어 반환한다.
// 다지역 확장 시 이 지점에 지역별 필터를 추가한다.
...
/** 격차 리포트 셸(FR-GR-01/03) + 연결맵(FR-GR-02/04) + 기회카드(FR-GR-05) 데이터 묶음. */
