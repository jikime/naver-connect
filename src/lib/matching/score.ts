...
// M1은 임베딩 없이 태그 교차 + 경량 한국어 토큰 겹침으로 계산한다(구조화 baseline).
// M2에서 dense 유사도가 이 함수들의 텍스트 항을 대체(shadow 비교)한다.
// 근거: people_match_retrieval_plan.md §6.2, plans/generic-mixing-seahorse.md M1-6

import type { PersonContextLite } from "@/lib/matching/engine";
import type { CapabilityOfferV1, ImpactIntentV1, NeedIntentV1 } from "@/types";
 ...
 * 태그 교차가 기본 신호(없으면 텍스트 겹침만으로 약한 경로), 최우선(★)·시급성은 승수.
 */
export function needOfferScore(
  need: NeedIntentV1,
  ...
  const text = textOverlap(need.detail_quote, offer.detail);
