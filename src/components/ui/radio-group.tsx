...
function RadioGroupItem({
  ...
  // children이 있으면 기본 원형 라디오닷 대신 그 내용을 그대로 렌더한다 — guud의
  // filter-chip-selected(pill, 채움 배경) 패턴으로 라디오 그룹을 스타일링할 때 쓴다
  // (예: onboarding CollaborationTraitsStep 단일선택 칩). 시맨틱(role=radio·단일선택·
  // 화살표 키 이동)은 RadioGroupPrimitive.Item이 그대로 제공한다.
  if (children) {
    return (
      <RadioGroupPrimitive.Item
        data-slot="radio-group-item"
        className={cn(
          "outline-none focus-visible:ring-2 focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50",
          className,
        )}
        {...props}
      >
