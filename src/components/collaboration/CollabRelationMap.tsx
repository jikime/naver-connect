...
const SUBGROUP_LABELS: Record<string, string> = {
  A1: "사회적기업형",   A2: "자원순환형",    A3: "지역밀착형",
  A4: "조합형",         A5: "인증기업형",    A6: "성장혁신형",
  B1: "자금공급형",     B2: "역량강화형",    B3: "인프라·네트워크형",
  B4: "대변·제도형",
  C1: "일반기업형",     C2: "전문서비스파트너형", C3: "글로벌·대기업형",
  C4: "학계·공공·언론형",
};
...
const LAYER = {
  A: {
    bg: "#edfaf4",       gradFrom: "#d4f0e5", gradTo: "#f0fbf6",
    border: "#34a871",   text: "#1a6b50",     node: "#34a871",
    edge: "#34a871",
  },
  B: {
    bg: "#fff8ed",       gradFrom: "#fde8c3", gradTo: "#fffcf5",
    border: "#e0982a",   text: "#7a4f10",     node: "#e0982a",
    edge: "#e0982a",
  },
  C: {
    bg: "#f0f3f8",       gradFrom: "#dde3ef", gradTo: "#f5f7fb",
    border: "#6a82a0",   text: "#3a4d64",     node: "#6a82a0",
    edge: "#6a82a0",
  },
} as const;
...
function buildLayout(
  ...
  const codeOf  = (id: string) => subgroupMap.find(e => e.org_id === id)?.subgroup_code ?? "A1";
  const layerOf = (code: string): LayerKey => code[0] === "A" ? "A" : code[0] === "B" ? "B" : "C";

  // 등장 org 수집 + 레이어 필터
  const orgIdSet = new Set<string>();
  for (const r of relations) { orgIdSet.add(r.org_a_id); orgIdSet.add(r.org_b_id); }
  ...
  function layoutLayer(
    ...
    for (const code of codes) {
      ...
      groups.push({ code, label: SUBGROUP_LABELS[code] ?? code, layer,
        x: COL_X[layer], y, w: COL_W, h: boxH, count: orgs.length });
...
function NodeDetail({ nodeId, orgs, relations, onClose }: {
  nodeId: string; orgs: Organization[];
  relations: CollabRelation[]; onClose: () => void;
}) {
  ...
  const connected = relations.filter(r => r.org_a_id === nodeId || r.org_b_id === nodeId);
    ...
    <div className="w-60 shrink-0 overflow-hidden rounded-2xl border border-guud-hairline bg-card text-xs shadow-md">
      <div className="px-4 pt-3 pb-2 flex items-start justify-between gap-2"
        ...
        <div>
          <span className="inline-block rounded px-1.5 py-0.5 text-[10px] font-black"
                style={{ background: lc.bg, color: lc.text }}>{code}</span>
          <p className="mt-1 font-semibold text-foreground leading-snug">{org?.name ?? nodeId}</p>
          <p className="text-guud-text-muted-2">{entry?.subgroup_label}</p>
        </div>
        <button type="button" onClick={onClose} aria-label="닫기"
                className="text-guud-text-muted-2 hover:text-foreground shrink-0">✕</button>
      </div>
      {org && (
      ...
      <div className="px-4 pb-3">
        ...
        <ul className="max-h-48 space-y-1 overflow-y-auto">
          {connected.sort((a, b) => b.strength - a.strength).map(r => (
            <li key={r.id} className="flex items-start gap-1.5 rounded-lg border border-guud-hairline p-1.5">
              <span className="mt-0.5 shrink-0"
                    style={{ color: r.is_actual ? "#16a34a" : "#d97706" }}>
                {r.is_actual ? "●" : "○"}
              </span>
              <div className="min-w-0">
                <p className="truncate font-medium text-foreground">{partnerName(r)}</p>
                <p className="text-guud-text-muted-2">{r.relation_type} · {Math.round(r.strength * 100)}점</p>
              </div>
            </li>
          ))}
        </ul>
...
function EdgeDetail({ relation, orgs, onClose }: {
  relation: CollabRelation; orgs: Organization[]; onClose: () => void;
}) {
  const nameA = orgs.find(o => o.id === relation.org_a_id)?.name ?? relation.org_a_id;
  const nameB = orgs.find(o => o.id === relation.org_b_id)?.name ?? relation.org_b_id;
  ...
  return (
    <div className="w-60 shrink-0 overflow-hidden rounded-2xl border border-guud-hairline bg-card text-xs shadow-md">
      <div className="px-4 pt-3 pb-2 flex items-start justify-between gap-2"
        ...
        <button type="button" onClick={onClose} aria-label="닫기"
                className="text-guud-text-muted-2 hover:text-foreground">✕</button>
      </div>
      <div className="px-4 py-3 space-y-2">
        <p className="text-guud-text-muted-2 leading-snug">{nameA}<br/>× {nameB}</p>
        <p className="text-foreground leading-relaxed">{relation.description}</p>
        <div className="flex flex-wrap gap-x-3 text-guud-text-muted-2 pt-0.5">
          <span>유형: <b className="text-foreground">{relation.relation_type}</b></span>
          <span>신뢰도: <b className="text-foreground">{Math.round(relation.strength * 100)}점</b></span>
        </div>
        <p>
          {relation.is_actual
          ...
          {relation.basis_case_id && (
            <span className="ml-2 text-guud-text-muted-2">근거: {relation.basis_case_id}</span>
          )}
        </p>
      </div>
    </div>
...
export function CollabRelationMap({
  relations, orgs, onSelectRelation,
}: {
  relations: CollabRelation[];
  orgs?: Organization[];
  onSelectRelation?: (rel: CollabRelation | null) => void;
}) {
  const resolvedOrgs = (orgs?.length ?? 0) > 0
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1 text-xs">
        <span className="font-semibold text-foreground">{nodes.length}개 기관 표시중</span>
        <span className="text-green-700">● 실제 협력 {actualCnt}건</span>
        <span className="text-amber-600">○ 잠재 협력 {potCnt}건</span>
        <span className="ml-auto text-guud-text-muted-2">노드·선 클릭 → 상세 정보</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-xl border border-guud-hairline bg-card px-4 py-2.5 text-xs">
        ...
        <label className="flex cursor-pointer items-center gap-1.5">
          <input type="checkbox" checked={filter.onlyActual}
            onChange={e => setFilter(f => ({ ...f, onlyActual: e.target.checked }))}
            className="h-3 w-3 rounded" />
          실제만
        </label>
        {/* 협업신뢰도 슬라이더 */}
        <label className="flex items-center gap-2 ml-auto">
          <span className="text-guud-text-muted-2 whitespace-nowrap">협업신뢰도 최소</span>
          <input type="range" min={0} max={1} step={0.05}
            value={filter.minStrength}
            onChange={e => setFilter(f => ({ ...f, minStrength: parseFloat(e.target.value) }))}
      ...
      <div className="flex gap-3 items-start">
        {/* SVG */}
        <div className="min-w-0 flex-1 overflow-x-auto rounded-2xl border border-guud-hairline shadow-sm" style={{ background: "#f4f7fb" }}>
          <svg
            viewBox={`0 0 ${VIEW_W} ${Math.max(viewH, 420)}`}
            style={{ width: "100%", height: "auto", minHeight: 360, display: "block" }}
            aria-label="협업관계 그래프" role="img"
          >
            <defs>
              {/* 레이어별 그라디언트 */}
              ...
              {(["A", "B", "C", "sel", "pot"] as const).map(k => {
                const color = k === "sel" ? "#e0982a" : k === "pot" ? "#b0bec5" : LAYER[k as LayerKey].edge;
                return (
                  <marker key={k} id={`arr-${k}`} markerWidth="5" markerHeight="5" refX="4.5" refY="2.5" orient="auto">
                    <polygon points="0 0,5 2.5,0 5" fill={color} opacity={k === "pot" ? 0.5 : 0.75} />
                  </marker>
                );
              })}
            </defs>
            ...
            {/* 레이어 컬럼 배경 */}
            {(["A", "B", "C"] as const).map(l => {
              if ((l === "A" && !filter.showA) || (l === "B" && !filter.showB) || (l === "C" && !filter.showC)) return null;
              return (
                <g key={l}>
                  <rect x={COL_X[l]} y={0} width={COL_W} height={Math.max(viewH, 420)}
                        fill={`url(#grad-${l})`} rx={10} opacity={0.45} />
                  {/* 컬럼 제목 */}
                  <rect x={COL_X[l] + 6} y={6} width={COL_W - 12} height={18} rx={5}
            ...
            {groups.map(g => {
              ...
              // 연결된 엣지 수
              const edgeCnt = filteredRels.filter(r =>
                (subgroupMap.find(e => e.org_id === r.org_a_id)?.subgroup_code === g.code) ||
                (subgroupMap.find(e => e.org_id === r.org_b_id)?.subgroup_code === g.code)
              ).length;

              return (
                <g key={g.code}>
                  <rect x={g.x + 4} y={g.y} width={g.w - 8} height={g.h} rx={7}
                        fill="rgba(255,255,255,0.72)" stroke={lc.border} strokeWidth={1}
                        strokeDasharray="4 2" />
                  {/* 헤더 라인 */}
                  <line x1={g.x + 4} y1={g.y + GROUP_HEAD} x2={g.x + g.w - 4} y2={g.y + GROUP_HEAD}
                        stroke={lc.border} strokeWidth={0.6} opacity={0.4} />
                  {/* 코드 */}
                  <rect x={g.x + 8} y={g.y + 4} width={22} height={14} rx={3}
                        fill={lc.border} opacity={0.85} />
                  <text x={g.x + 19} y={g.y + 14} textAnchor="middle"
                        fontSize="8" fontWeight="900" fill="#fff">{g.code}</text>
                  {/* 라벨 */}
                  <text x={g.x + 34} y={g.y + 14} fontSize="8.5" fontWeight="600" fill={lc.text}>
                    {g.label}
                  </text>
                  {/* 연결 건수 뱃지 */}
                  {edgeCnt > 0 && (
                    <g>
                      <rect x={g.x + g.w - 30} y={g.y + 4} width={22} height={14} rx={7}
                            fill={lc.border} opacity={0.18} />
                      <text x={g.x + g.w - 19} y={g.y + 14} textAnchor="middle"
                            fontSize="8" fontWeight="700" fill={lc.text}>{edgeCnt}건</text>
                    </g>
                  )}
                </g>
              );
            ...
            {filteredRels.map(r => {
              ...
              const strokeBase = r.is_actual ? LAYER[layerA]?.edge ?? "#888" : "#b0bec5";
              ...
              const opacity = isSel ? 1 : isHov ? 0.9 : r.is_actual ? 0.55 : 0.35;
              const marker  = isSel ? "arr-sel" : r.is_actual ? `arr-${layerA}` : "arr-pot";
              ...
              return (
                <g key={r.id}>
                  ...
                  <path d={d} stroke="transparent" strokeWidth={14} fill="none"
                        ...
                        aria-label={`${r.relation_type} 관계`} tabIndex={0}
                        onClick={() => clickEdge(r)}
                        onMouseEnter={() => setHovRel(r.id)}
                        onMouseLeave={() => setHovRel(null)}
                        onKeyDown={e => { if (e.key === "Enter" || e.key === " ") clickEdge(r); }} />
                  <path d={d} stroke={stroke} strokeWidth={sw} strokeDasharray={dash}
                        opacity={opacity} fill="none" markerEnd={`url(#${marker})`}
            ...
            {nodes.map(n => {
              ...
              return (
                ...
                <g key={n.id} className="cursor-pointer" role="button"
                   aria-label={label} tabIndex={0}
                   onMouseEnter={() => setHovNode(n.id)}
                   onMouseLeave={() => setHovNode(null)}
                   onClick={() => clickNode(n.id)}
                   onKeyDown={e => { if (e.key === "Enter" || e.key === " ") clickNode(n.id); }}>

                  {/* 후광 */}
                  {(isSel || isHov) && (
                    <circle cx={n.cx} cy={n.cy} r={r + 5}
                            fill={lc.border} opacity={0.15}
                            className="pointer-events-none" />
                  )}

                  {/* 흰색 테두리 (배경과 구분) */}
                  <circle cx={n.cx} cy={n.cy} r={r + 1.5}
                  ...
                  <text x={n.cx} y={n.cy + 3.5} textAnchor="middle"
                        ...
                        className="pointer-events-none select-none">{n.code}</text>

                  {/* 기관명 레이블 배경 */}
                  <rect
                    x={n.cx - labelW / 2} y={n.cy + r + 3}
                    width={labelW} height={LABEL_LINE_H}
                    rx={3} fill="rgba(255,255,255,0.85)"
                    className="pointer-events-none" />
                  {/* 기관명 */}
                  <text x={n.cx} y={n.cy + r + LABEL_LINE_H}
                        textAnchor="middle"
                        fontSize={LABEL_FONT}
        ...
        {selectedNodeId && !selRelObj && (
          <NodeDetail nodeId={selectedNodeId} orgs={resolvedOrgs}
                      relations={relations} onClose={() => setSelNode(null)} />
        )}
        {selRelObj && !selectedNodeId && (
          <EdgeDetail relation={selRelObj} orgs={resolvedOrgs}
                      onClose={() => { setSelRel(null); if (onSelectRelation) onSelectRelation(null); }} />
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-[11px] text-guud-text-muted-2">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-px w-6 border-t-2" style={{ borderColor: LAYER.A.edge }} />
          실제 협력 (선 굵기 = 신뢰도)
        </span>
        <span className="flex items-center gap-1.5">
