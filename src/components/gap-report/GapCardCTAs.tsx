...
// 추천발송·모둠개설은 백엔드가 없어(§7 Out of Scope) 접수 안내만 표시하는 목업 인터랙션이다.
...
type CtaType = "추천발송" | "모둠개설" | "3단계협업지원";
...
const MOCK_ACK: Record<"추천발송" | "모둠개설", string> = {
  추천발송: "추천 발송 요청을 접수했습니다(목업 — 실제 발송 없음).",
  모둠개설: "모둠 개설 요청을 접수했습니다(목업 — 실제 개설 없음).",
};

export function GapCardCTAs({ gapCard }: { gapCard: GapCard }) {
  const [ackType, setAckType] = useState<"추천발송" | "모둠개설" | null>(null);
